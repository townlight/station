...
`station-media-assets` probes actual streams and finite duration, rejects non-media or incomplete A/V, copies accepted bytes atomically into a SHA-256-addressed library, revalidates the stored object, and makes duplicate imports idempotent. `station-media-engine` can then add that stored file to an already-running fallback graph. The file's source pads are aligned to the live running clock, both selectors feed one continuous segment, and video/audio are normalized before the persistent OpenH264/AAC encoders and MPEG-TS output. The integration proof generates a real A/V file, ingests it, dynamically loads it, switches fallback → asset → fallback, and rejects transport errors, discontinuities, counter jumps, backward PTS, or excessive A/V divergence. Full schedule automation, successive asset replacement, live inputs, production profiles, and sink fanout are not claimed yet.
