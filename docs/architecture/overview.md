...
`station-media-engine` owns the GStreamer graph. It builds all elements through factories and leaves OpenH264, AAC, the parsers, `mpegtsmux`, and UDP output in one persistent graph across switches. A validated stored asset can be decoded and added while fallback remains on air. Its pads receive the running pipeline offset, both selectors feed single continuous timelines, and a sample-domain audio rate adjuster prevents boundary jitter. The Windows machine test receives the actual transport stream and validates sync bytes, packet boundaries, declared H.264 and AAC streams, transport-error and discontinuity flags, per-PID continuity counters, strict PTS monotonicity, and bounded A/V timing across fallback → real file → fallback. See [ADR 0004](../adr/0004-persistent-gstreamer-graph.md).
