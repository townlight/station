...
`station-media-engine` owns the GStreamer graph. Its first executable slice builds all elements through factories, feeds paired synthetic fallback and program A/V legs into clock-synchronized selectors at identical raw caps, and leaves OpenH264, AAC, the parsers, `mpegtsmux`, and UDP output in one persistent graph across switches. A sample-domain audio rate adjuster prevents rounding jitter at selection boundaries. The Windows machine test receives the actual transport stream and validates sync bytes, packet boundaries, declared H.264 and AAC streams, transport-error and discontinuity flags, per-PID continuity counters, strict PTS monotonicity, and bounded A/V timing. See [ADR 0004](../adr/0004-persistent-gstreamer-graph.md).
...
The first channel worker now runs as an independent supervised process and owns its persistent GStreamer graph. Durable `Ready` follows successful fallback output startup; durable `ShutdownComplete` follows graph shutdown. The supervisor proves that the real worker produces MPEG-TS after handshake, while the engine test separately proves uninterrupted A/V continuity across fallback/program switches. Real media/live inputs, profile-driven output, and `stationd` channel configuration are the next media milestones.
